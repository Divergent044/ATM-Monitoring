export const headerTitle = "ATM Cash Management";

export const instituteTableTitle = ["Идентификатор", "Описание"];

export const groupsTableTitle = ["Название группы", "Описание группы"];

export const optionButtonText = {
    add: "Добавить",
    edit: "Изменить",
    remove: "Удалить",
    addGroup: "Добавить группу",
    editGroup: "Изменить группу",
    removeGroup: "Удалить группу",
    save: "Сохранить",
    cancel: "Отмена",
    ok: "ОК"
};

export const addInstitutePopup = {
    title: "Добавить Институт",
    idLabel: "Идентификатор",
    descriptionLabel: "Описание"
};

export const addGroupPopup = {
    title: "Добавить Группу",
    idLabel: "Название группы",
    descriptionLabel: "Описание группы"
};

export const editInstitutePopup = {
    title: "Измените описание Института",
    idLabel: "Идентификатор",
    descriptionLabel: "Описание"
};

export const editGroupPopup = {
    title: "Измените описание группы",
    idLabel: "Название группы",
    descriptionLabel: "Описание"
};

export const removeInstitutePopup = {
    titleBeforeId: "Инстиут",
    titleAfterId: "будет удален, Вы уверены ?",
};

export const removeGroupPopup = {
    titleBeforeId: "Группа",
    titleAfterId: "будет удалена, Вы уверены ?",
};
